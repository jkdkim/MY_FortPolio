...
<c:forEach var="i" begin="0" end="${cate.size()-1 }" varStatus="status">
			<div class="w3-container w3-card-2 w3-white w3-round w3-margin">
				<br> <img
					src="/profilefolder/${cate[i].get('ID')}" alt="Avatar"
				...
				<input type="hidden" id="uploadId"
					value="${cate[i].get('FILEUUID')}"> #업로드된 이  파일번호 는 [<b>${cate[i].get('FILEUUID')}</b>]입니다
				<div class="w3-row-padding" style="margin: 0 -16px">
					<div class="w3-half">
						<button type="button" id="filemodal_${status.index }"
							style="border: none; background: transparent;"
							onclick="button1_click('${cate[i].get('FILEUUID') }', '${cate[i].get('ID')}','${cate[i].get('COMMENTS') }','${cate[i].get('CATE') }','${userId }' )">
						<img src="/users/${cate[i].get('FILEUUID') }" style="width: 100%" alt="Northern Lights" class="w3-margin-bottom" id="test">
						</button>
					</div>
				</div>
				...
				<input type="text" id="f_comments_${cate[i].get('FILEUUID') }"
					...
					onblur="board_blur('${cate[i].get('FILEUUID') }','${nickname }','${userId }')">
